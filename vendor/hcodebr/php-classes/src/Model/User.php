...
class User extends Model {
	...
	const SESSION = "User";
	const SECRET = "HcodePhp7_Secret"; //mudar quando hospedar. Mesmo quantidade de caracteres
	...
	public static function login($login, $password)
	{
		...
		if (password_verify($password, $data["despassword"]) === true)
		{
			...
			$_SESSION[User::SESSION] = $user->getValues(); //coloca dados do Usuario na sessao

			return $user;

		} else {
			throw new \Exception("Usuário inexistente ou senha inválida."); //Classe nivel abaixo
		}
	...
	public static function getForgot($email, $inadmin = true)
	{
		...
		if (count($results) === 0)//email nao encontrado no banco
		{
		...
		{//email encontrado
			...
			$results2 = $sql->select("CALL sp_userspasswordsrecoveries_create(:iduser, :desip)", array(
				":iduser"=>$data['iduser'],
				":desip"=>$_SERVER['REMOTE_ADDR']//pega IP 
			));
...
public static function validForgotDecrypt($code)
	{
		...
		$code = base64_decode($code);//decriptar 
	...
	public static function getPage($page = 1, $itemsPerPage = 10)//itens por página
	{

		$start = ($page - 1) * $itemsPerPage; //1-1*3 = 0 | 2 -1 *3 = 3
		...
		return [
			...
			'pages'=>ceil($resultTotal[0]["nrtotal"] / $itemsPerPage) //ceil converte arredondando para cima
		];
